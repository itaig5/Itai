...
export async function GET() {
  ...
  const listings: PortfolioResponse['listings'] = state.listings.map((l) => {
  ...
  const totals = {
    ...
    activePromos: state.promotions.filter((p) => p.status === 'active').length,
    openRecs: state.recommendations.filter((r) => (r.status ?? 'proposed') === 'proposed').length,
    pendingOutcomes: state.outcomes.filter((o) => o.status === 'pending').length,
  };
