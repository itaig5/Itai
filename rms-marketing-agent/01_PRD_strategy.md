...
| **Antitrust / auto-pricing liability** | 🟠 Med | ✅ *Gibson v. Cendyn* (9th Cir., Aug 15 2025) dismissed an antitrust claim against pricing software **because** (a) recommendations were **non-binding** and (b) there was **no pooling of competitors' confidential data**. ✅ The DOJ–RealPage settlement (Nov 2025) punished the opposite. **→ Product rules: advisory (user confirms), public/market data only, never pool confidential pricing across competing clients, no "auto-accept" defaults, occupancy/pace triggers use the property's OWN data only, and never deliver *aligned* recommendations to competitors in one micro-market.** |
| **State pricing-law exposure** (new — Gemini cross-check `09`) | 🟠 Med | **California AB 325 + SB 763** (Cartwright Act, eff. Jan 2026): statutory liability for a **"common pricing algorithm"** used by ≥2 competitors incorporating **shared competitor info**; lowered pleading bar; fines up to **$6M or 2× gain + treble damages**. **NY Algorithmic Pricing Disclosure Act** (eff. Nov 2025): disclosure required if pricing uses personal/consumer-profile data. **→ Public-market-data-only + no subscriber-data pooling keeps you outside the "common algorithm" definition; price on property/market signals, not consumer profiles.** |
